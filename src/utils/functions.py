...
class data_processing():
    ...
    def procesado_cuotas(self, file_names, df_ids):
        ...
        def select_columns_and_add_season(df, file_name):
            ...
            columns = ['HomeTeam', 'AwayTeam', 'B365H', 'B365D', 'B365A']

            if 'B365>2.5' in df.columns and 'B365<2.5' in df.columns:
                columns.extend(['B365>2.5', 'B365<2.5'])
                df_selected = df[columns]
                df_selected.rename(columns={'B365>2.5': 'odd_mas_25', 'B365<2.5': 'odd_menos_25'}, inplace=True)

            elif 'BbMx>2.5' in df.columns and 'BbMx<2.5' in df.columns:
                columns.extend(['BbMx>2.5', 'BbMx<2.5'])
                df_selected = df[columns]
                df_selected.rename(columns={'BbMx>2.5': 'odd_mas_25', 'BbMx<2.5': 'odd_menos_25'}, inplace=True)

            # Añadir columna "season" con el año extraído
            df_selected['season'] = int(year)

            #cambiar el nombre a las columnas B365H...

            df_selected.rename(columns={'B365H': 'odd_1', 'B365D': 'odd_x', 'B365A':'odd_2'}, inplace=True)
        ...
        for file_name in file_names:
            ...
            df_processed = select_columns_and_add_season(df, file_name)
    ...
    def creacion_nuevas_variables(self, df_final):
        ...
        df_final['goles_totales'] = df_final['goles_local'] + df_final['goles_visitante']
    ...
    def creacion_datos_nuevos(self, df_partidos,id_equipo_local, id_equipo_visitante,
